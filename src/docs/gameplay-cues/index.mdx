...
Upon receiving a gameplay event (like it's done in the [Handling Collision](/guides/collision/) guide), the Cues Container matching the Gameplay Event Tag will be applied. If the `Gameplay Cue Tags` is matching a Gameplay Cue Blueprint, the Gameplay Cue Manager should execute them appropriately.
