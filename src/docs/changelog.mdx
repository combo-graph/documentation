...
* Expose bStopWhenAbilityEnds bool to Combo Nodes making it possible to prevent montage being aborted if the ability ends normally in [#50](https://github.com/combo-graph/combo-graph/pull/50)

### Bug Fixes

* Fix to prevent assertion crashes if ability is explicitly ended on EventReceived event in [#51](https://github.com/combo-graph/combo-graph/pull/51)
...
- Changes for 5.3-preview in [#49](https://github.com/combo-graph/combo-graph/pull/49)
    ...
    - [Fix 5.3 deprecation warning about SetStackCount() in UComboGraphAbilityTask_StartGraph](https://github.com/combo-graph/combo-graph/pull/49/commits/36c97d6a6b26ca2d5a682cd3994eecdaf28b63c0)
...
* Fix Lyra Edge replication by not duplicating edges, and remove 4.26 / 4.27 / 5.0 workaround with Edge DuplicateObject in [#47](https://github.com/combo-graph/combo-graph/pull/47)
* Fix preview mesh picker for 5.2 by setting FPersonaToolkitArgs::bPreviewMeshCanUseDifferentSkeleton in [#48](https://github.com/combo-graph/combo-graph/pull/48)

Thanks to Killer_Whaley and SirSuperDude on Discord for the bug reports.

Note that this release drops support for engine version prior to 5.0 (so 4.26 / 4.27 / 5.0), which means going forward only the last two versions of the engines will receive further updates.
...
* Fixes a deprecation warning in 5.2 in [#46](https://github.com/combo-graph/combo-graph/pull/46)
