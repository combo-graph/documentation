...
The plugin relies on a few Gameplay Tags, namely to send and listen to specific gameplay event within ability tasks.
Those are native tags created by the plugin and are meant for internal uses (you can't remove them from the editor).
